...
static LINKER_SCRIPT_FOOTER_TEMPLATE: &str = r#"
    ...
    P: AsRef<Path>,
{
    let filename = path
        ...
        .to_string_lossy();
    let dir = path.as_ref().parent().ok_or("invalid path for linker script")?.to_string_lossy();
...
fn generate_footer<'a>(
    ...
    match cfg.max_size {
        ...
        Some(size) => {
            let footer_templ = Template::new(LINKER_SCRIPT_FOOTER_TEMPLATE);
